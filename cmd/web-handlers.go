...
func (web *webAPIHandlers) DeleteBucket(r *http.Request, args *RemoveBucketArgs, reply *WebGenericRep) error {
	...
	if web.CacheAPI(m) != nil {
		deleteBucket = web.CacheAPI(m).DeleteBucket
	}
...
func (web *webAPIHandlers) ListBuckets(r *http.Request, args *WebGenericArgs, reply *ListBucketsRep) error {
	...
	if web.CacheAPI(m) != nil {
		listBuckets = web.CacheAPI(m).ListBuckets
	}
...
func (web *webAPIHandlers) ListObjects(r *http.Request, args *ListObjectsArgs, reply *ListObjectsRep) error {
	...
	if web.CacheAPI(m) != nil {
		listObjects = web.CacheAPI(m).ListObjects
	}
...
next:
	for _, objectName := range args.Objects {
		...
		if !hasSuffix(objectName, slashSeparator) && objectName != "" {
			...
			if err = deleteObject(nil, objectAPI, web.CacheAPI(m), args.BucketName, objectName, r); err != nil {
				break next
			}
			continue
		}
		...
		for {
			...
			for _, obj := range lo.Objects {
				err = deleteObject(nil, objectAPI, web.CacheAPI(m), args.BucketName, obj.Name, r)
				if err != nil {
					break next
				}
			}
...
func (web *webAPIHandlers) SetAuth(r *http.Request, args *SetAuthArgs, reply *SetAuthReply) error {
	...
	if err = saveServerConfig(newObjectLayerFn(m), globalServerConfig); err != nil {
...
func (web *webAPIHandlers) Upload(w http.ResponseWriter, r *http.Request) {
	...
	if web.CacheAPI(m) != nil {
		putObject = web.CacheAPI(m).PutObject
	}
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
...
func (web *webAPIHandlers) Download(w http.ResponseWriter, r *http.Request) {
	...
	if web.CacheAPI(m) != nil {
		getObject = web.CacheAPI(m).GetObject
	}
	getObjectInfo := objectAPI.GetObjectInfo
	if web.CacheAPI(m) != nil {
		getObjectInfo = web.CacheAPI(m).GetObjectInfo
	}
